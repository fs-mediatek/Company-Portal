...
export async function GET() {
  try {
    ...
    // Add location_id to tickets if not exists
    try {
      await query(`ALTER TABLE tickets ADD COLUMN location_id INT UNSIGNED DEFAULT NULL AFTER location_room`)
    } catch {}
    ...
    // Kommissionierung (commissioning orders)
    await query(`CREATE TABLE IF NOT EXISTS commissions (
      ...
      recipient_street VARCHAR(200) DEFAULT NULL,
    ...
    // Add unit_price to commission_items if not exists
    try {
      await query(`ALTER TABLE commission_items ADD COLUMN unit_price DECIMAL(10,2) DEFAULT 0.00 AFTER picked_quantity`)
    } catch {}

    // Abrechnungen (invoices)
    await query(`CREATE TABLE IF NOT EXISTS commission_invoices (
      id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
      invoice_number VARCHAR(30) NOT NULL UNIQUE,
      month INT NOT NULL,
      year INT NOT NULL,
      group_type ENUM('gesamt','empfaenger','standort') DEFAULT 'gesamt',
      group_value VARCHAR(200) DEFAULT NULL,
    ...
    await query(`CREATE TABLE IF NOT EXISTS invoice_counters (
      year INT PRIMARY KEY,
      last_number INT DEFAULT 0
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`)
    ...
    // Seed built-in roles
    const builtinRoles = [
      { name: 'admin', label: 'Administrator', color: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400', sort: 1 },
      { name: 'manager', label: 'Manager', color: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400', sort: 2 },
      { name: 'techniker', label: 'Techniker', color: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400', sort: 3 },
      { name: 'hausmeister', label: 'Hausmeister', color: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400', sort: 4 },
      { name: 'melder', label: 'Melder', color: 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400', sort: 5 },
    ]
