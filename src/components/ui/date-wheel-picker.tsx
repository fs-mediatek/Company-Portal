...
function getMonthNames(locale?: string): string[] {
  const formatter = new Intl.DateTimeFormat(locale || "de-DE", { month: "long" });
...
function WheelColumn({
  ...
  React.useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (newIndex !== currentValue) onChangeRef.current(newIndex);
    };
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    let newIndex = value;
    switch (e.key) {
      case "ArrowUp": e.preventDefault(); newIndex = Math.max(0, value - 1); break;
      case "ArrowDown": e.preventDefault(); newIndex = Math.min(maxIndex, value + 1); break;
      case "Home": e.preventDefault(); newIndex = 0; break;
      case "End": e.preventDefault(); newIndex = maxIndex; break;
      case "PageUp": e.preventDefault(); newIndex = Math.max(0, value - 5); break;
      case "PageDown": e.preventDefault(); newIndex = Math.min(maxIndex, value + 5); break;
      default: return;
    }
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative overflow-hidden", disabled && "opacity-50 pointer-events-none", className)}
    ...
    >
      <div
        ...
        style={{ height: centerOffset, background: "linear-gradient(to bottom, var(--background) 0%, transparent 100%)" }}
        aria-hidden="true"
      />
      <div
        className="absolute inset-x-0 bottom-0 z-10 pointer-events-none"
        style={{ height: centerOffset, background: "linear-gradient(to top, var(--background) 0%, transparent 100%)" }}
        aria-hidden="true"
      />
      <div
...
const DateWheelPicker = React.forwardRef<HTMLDivElement, DateWheelPickerProps>(
  ({ value, onChange, minYear = 1920, maxYear = new Date().getFullYear(), size = "md", disabled = false, locale = "de-DE", className, ...props }, ref) => {
    ...
    const handleMonthChange = React.useCallback((monthIndex: number) => {
      ...
      setDateState(prev => {
        ...
        return { ...prev, month: monthIndex, day: Math.min(prev.day, daysInNewMonth) };
      });
    }, []);
    ...
    const handleYearChange = React.useCallback((yearIndex: number) => {
      ...
      setDateState(prev => {
        ...
        return { ...prev, year: newYear, day: Math.min(prev.day, daysInNewMonth) };
      });
    }, [years]);
    ...
    React.useEffect(() => {
      if (value && !isInternalChange.current) {
        const vd = value.getDate(), vm = value.getMonth(), vy = value.getFullYear();
        if (vd !== dateState.day || vm !== dateState.month || vy !== dateState.year) {
          setDateState({ day: vd, month: vm, year: vy });
        }
      }
    ...
    return (
      <div
        ref={ref}
        className={cn("flex items-center justify-center", config.gap, config.fontSize, disabled && "opacity-50 pointer-events-none", className)}
        style={{ perspective: "1000px" }}
        role="group"
        aria-label="Datumsauswahl"
        {...props}
      >
        <WheelColumn items={days} value={dateState.day - 1} onChange={handleDayChange} itemHeight={config.itemHeight} visibleItems={VISIBLE_ITEMS} disabled={disabled} className="w-16" ariaLabel="Tag auswählen" />
        <WheelColumn items={months} value={dateState.month} onChange={handleMonthChange} itemHeight={config.itemHeight} visibleItems={VISIBLE_ITEMS} disabled={disabled} className="w-28" ariaLabel="Monat auswählen" />
        <WheelColumn items={years} value={yearIndex >= 0 ? yearIndex : 0} onChange={handleYearChange} itemHeight={config.itemHeight} visibleItems={VISIBLE_ITEMS} disabled={disabled} className="w-20" ariaLabel="Jahr auswählen" />
      </div>
    );
  }
);
