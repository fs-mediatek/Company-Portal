...
async function getMailSettings(): Promise<MailSettings> {
  ...
  const rows = await coreQuery("SELECT key_name, value FROM settings WHERE key_name LIKE 'smtp_%'") as any[]
  const s: any = {}
...
export async function createTransport() {
  ...
  const secure = s.smtp_secure === "ssl"
  ...
  const options: any = {
    ...
    port,
    secure,
    tls: { rejectUnauthorized: s.smtp_reject_unauthorized !== "false" },
  }
  ...
  if (s.smtp_user && s.smtp_pass) {
    options.auth = { user: s.smtp_user, pass: s.smtp_pass }
  }

  return { transport: nodemailer.createTransport(options), from: s.smtp_from || "portal@localhost" }
}
