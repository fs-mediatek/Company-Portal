...
import { coreQuery, coreQueryOne, coreInsert } from "@/lib/core-db"
import bcrypt from "bcryptjs"

async function getBaseUrl(fallback: string): Promise<string> {
  try {
    const rows = await coreQuery("SELECT value FROM settings WHERE key_name = 'app_url'") as any[]
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Find or create user in portal_core
    let user = await coreQueryOne<any>("SELECT * FROM users WHERE email = ?", [email])

    if (!user) {
      // Auto-create user on first MS login
      const randomPass = crypto.randomUUID()
      const hash = await bcrypt.hash(randomPass, 10)

      await coreInsert(
        "INSERT INTO users (name, email, password_hash, role, phone, active) VALUES (?, ?, ?, ?, ?, 1)",
        [name, email, hash, "user", phone]
      )

      user = await coreQueryOne<any>("SELECT * FROM users WHERE email = ?", [email])
    } else if (!user.active) {
      return NextResponse.redirect(new URL("/login?error=Konto+ist+deaktiviert", baseUrl))
    } else {
      // Update name/phone from Microsoft profile
      await coreQuery(
        "UPDATE users SET name = ?, phone = COALESCE(?, phone) WHERE id = ?",
        [name, phone, user.id]
      )
    }
    ...
    const response = NextResponse.redirect(new URL("/select", baseUrl))
